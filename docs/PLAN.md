...
Phase 2  백엔드 API + DB              ✅ 완료
Phase 3  프론트-백 연동               ✅ 완료 ← 지금 여기
Phase 4  AI 에이전트 통합
...
### 1.5-3. 노션 데이터 연동 ✅ 완료 (Phase 2와 병행)

- [x] 노션 raw 데이터 → PostgreSQL 마이그레이션 (249세션, 961로그, 2599세트)
- [x] Mock 데이터 → 실제 DB 데이터로 교체 (Phase 3 API 연동 과정에서 완료)
- [x] 데이터 구조 검증 및 타입 정합성 확인
...
## 6. Phase 3 — 프론트-백 연동 ✅ 완료

**목표:** Mock 데이터를 실제 API로 교체

- [x] API 클라이언트 설정 (`fetch` 래퍼, `src/lib/api.ts`)
- [x] 각 페이지의 Mock 데이터 → API 호출로 전환 (운동 기록, 라이브러리, 캘린더 등)
- [x] 로딩/에러 상태 UI 처리
- [x] 낙관적 업데이트 (운동 기록 저장 시)
- [x] 오프라인 폴백 (운동 저장 실패 시 로컬 스토리지 큐잉 + 재시도)

### Phase 3 추가 완료 항목

- [x] `equipment` 컬럼 VARCHAR → VARCHAR[] 배열 전환 (Alembic 마이그레이션)
  - 다중 장비 지원 (예: 벤치프레스→{바벨,덤벨})
  - `EQUIPMENT_LIST` 상수 + `EquipmentName` 타입 (`constants.ts`)
- [x] 운동 타입별 기록 UI 분기 (유산소 → 시간/거리, 스트레칭 → 시간/세트)
- [x] 운동 세션 `duration_min` 기록 (총 운동시간 자동 산출)
  - 운동 완료 시 타이머 기반 자동 계산 → DB 저장
  - Log 화면: Clock 아이콘 + 시간 표시 (예: `45m`, `1h 30m`)
- [x] 이전 수행 기록 자동입력 (같은 운동의 최근 세트 데이터 자동 채움)
- [x] 운동 라이브러리 필터 UX 개선 (맨몸/없음 칩 숨김, 장비→기구 라벨)
- [x] 디자인 토큰 정비 + 체성분 홈 카드 추가
- [x] 배포 스크립트 다운타임 개선 (무중단 배포)
